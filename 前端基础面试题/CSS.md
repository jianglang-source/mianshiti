...
border-box: border/padding不会撑开元素 ，希望整个元素的宽高保持不变，但是里面的内容宽高可以发生改变的时候使用             

content-box：border/padding的设置会撑开元素，希望内容的宽高不变，可以元素的宽高发生变化。
...
### Flex
flex容器中存在两条轴， 主轴和侧轴， 容器中的每个单元称为flex item。
